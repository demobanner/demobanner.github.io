...
Cube.prototype.parse = function(options) {
    ...
    this.autoRotateAngle = options.autoRotateAngle;
...
Cube.prototype.rotate = function(fromAngle) {
  ...
  if (fromAngle == -1) {
    ...
    obj = { angle: this.autoRotateAngle[this.curLoop]};
  }

  this.curLoop ++;
  var toAngle = this.autoRotateAngle[this.curLoop];
  ...
  face_out = this.cube.getElementsByClassName(face)[0].querySelector('.clickTag');
  ...
  //}
  TweenMax.to(face_out, .8, {
    delay: this.autoRotateDelay + 0.5,
    alpha:0
  });
